/* 
Save Neeru Bajwa from criminals 
♥
*/
...
var height = window.innerHeight > 700 ? 700 : window.innerHeight;
var gameScore = 0;
var highScore = 0;
...
var SantaGame = {
	...
	load: {
		preload: function(){
			...
			// this.game.load.audio('ho-ho-ho', 'audio/Fall Sound.wav');
			// this.game.load.audio('hop', 'audio/jump-sound.mp3');
			this.game.load.image('platform', 'images/Ground.png');
			this.game.load.spritesheet('santa-running', 'images/avatar.png', 84, 105);
			this.game.load.image('snow-bg', 'images/Neeru Game BG.jpg');
			this.game.load.image('snowflake', 'images/Dust.png');
			this.game.load.image("logo", "images/Save Neeru From Criminals.png");
			this.game.load.image("instructions", "images/Instructions 584x239.png");
			...
			this.game.load.image("restartBtn", "images/Restart.png");
		},
		create: function(){
...
// instructions screen
	...
	play: {
          ...
          highScore = gameScore > highScore ? Math.floor(gameScore) : highScore;
				...
				this.platforms.createMultiple(8, 'platform', 0, false);
				...
				for(var i=0; i<5; i++){
					...
					plat.reset(i * 192, this.game.world.height - 30);
					plat.width = 192;
					plat.height = 30;
				...
				this.santa.body.gravity.y = 2000;
	...
	gameOver: {
		...
		create: function(){
			this.bg = this.game.add.tileSprite(0, 0, width, height, 'snow-bg');
			this.msg = this.game.add.sprite(this.game.world.width/2 - 240.5, 60, 'game-over');
			this.msg.alpha = 0;
			this.game.add.tween(this.msg).to({alpha: 1}, 600, Phaser.Easing.Linear.None, true, 0);

			this.score = this.game.add.text(this.game.world.width/2 - 60, 170, 'Score: ' + Math.floor(gameScore), { font: "32px Arial", fill: "white" });
			this.score.alpha = 0;
			this.game.add.tween(this.score).to({alpha: 1}, 500, Phaser.Easing.Linear.None, true, 600);

			this.restartBtn = this.game.add.button(this.game.world.width/2 - 130.5, 210, 'restartBtn', this.restartClicked);
